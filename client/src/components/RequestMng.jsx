...
const RequestMng = ({ request = null, onRequestChange,mat=null }) => {
    ...
    const [utilisateur, setUtilisateur] = useState(request ? request.utilisateur._id : '');
    const [material, setMaterial] = useState(request ? request.matériel._id : '');
    const [prise, setPrise] = useState(request ? request.prise : '');
    const [retour, setRetour] = useState(request ? request.retour : '');
    ...
    useEffect(() => {
        if (request) {
            setUtilisateur(request.utilisateur._id || '');
            setMaterial(request.matériel._id || '');
            setPrise(request.prise || '');
            setRetour(request.retour || '');
        } else {
            setUtilisateur('');
            setMaterial('');
            setPrise('');
        ...
        <>
            <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                    {request != null ? "Editer Demande" : "Ajouter Demande"}
                </h3>
            </div>
